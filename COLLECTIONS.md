...
## Add a whole TV show from Libraries (v1.1.40)

You no longer need to build a search just to add one known series. Open **Media
-> Libraries**, browse a library, click the TV show, then use **Add Entire Show
To Collection**. Choose an existing manual Collection or type a new Collection
name. ViperTV stores the whole-show selection, so later newly-synced episodes of
that show are included automatically.


## Adding Movies From the Library Browser

Open **Media → Movies**, or browse a library under **Media → Libraries**, and click a movie title. The movie metadata page has **Add Movie To Collection**. Choose an existing manual Collection or type a new Collection name to create it immediately. The exact movie is added; it does not broaden into the whole movie library.
