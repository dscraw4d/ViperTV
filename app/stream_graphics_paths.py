...
def _advanced_media_command_v129(channel, item: dict[str, Any], offset: float, input_args: list[str], profile_override: str | None = None) -> list[str]:
    ...
    if profile in {'vaapi','qsv'}:
        ...
        base += (['-vaapi_device',str(prereq.get('device') or preferred or '/dev/dri/renderD128')] if profile=='vaapi' else ['-qsv_device',str(prereq.get('device') or preferred or '/dev/dri/renderD128')])
...
def _path_page(msg: str = '') -> str:
    ...
    body=(f"<div class='msg'>{_e(msg)}</div>" if msg else '')+_heading('Plex Stream From Disk / Path Replacements','Translate the path stored by Plex into a path visible inside the ViperTV container. Plex metadata stays authoritative, but FFmpeg reads the media directly from disk when the translated file exists.',"<a class='button secondary' href='/plex'>Back to Plex</a>")+f"""
...
<div class='card'><h2>Add Replacement</h2><form method='post' action='/plex/path-replacements/add'><div class='grid'><div><label>Plex Server</label><select name='server_id' required>{sopts}</select><label>Plex path prefix</label><input name='remote_prefix' required placeholder='/volume1/Media or D:\\Media'></div><div><label>ViperTV container path prefix</label><input name='local_prefix' required placeholder='/mnt/share2'><label>Priority</label><input type='number' name='priority' value='100'></div></div><button>Add Replacement</button></form><p class='muted small'>This page never changes Docker/OMV mounts. The destination path must already be visible in the ViperTV container.</p></div>
<div class='card'><h2>Replacement Rules</h2><div class='table-wrap'><table><thead><tr><th>Server</th><th>Plex Prefix</th><th>Container Prefix</th><th>Priority</th><th>Status</th><th></th></tr></thead><tbody>{ro}</tbody></table></div></div>
...
def install_v129(app, main_globals: dict[str, Any]) -> None:
    ...
    def plex_page_v129(message: str=''):
        ...
        button="<div class='card'><h2>Direct Media Paths</h2><p>Optionally translate Plex's original file paths to ViperTV container paths so FFmpeg reads media directly from disk while Plex remains the metadata source.</p><a class='button secondary' href='/plex/path-replacements'>Manage Path Replacements</a></div>"
    ...
    def v129_path_test(server_id:int=Form(...),remote_path:str=Form(...)):
        ...
        msg=('Matched: '+x) if x else 'No enabled rule produced an existing file inside the ViperTV container.'
